  ...
  function saveCity() {
    ...
    if (!ls) {
      ...
      if (!cArr.includes(props.loc.city.name)) {
        ...
        alert('Already Added into the list. !!!')
      }
    }
...
function Information(props = '') {
  return (
    <section className={`${props.class} text-white w-3/4 flex flex-col justify-between`}>
      <section className='2xl:mb-8'>
        ...
        <p className='text-gray-300 2xl:text-2xl lg:text-lg sm:text-md'>{props.loc.list[0].weather[0].description}</p>
      </section>
      <section>
        <h1 className='2xl:text-5xl lg:text-5xl sm:text-3xl font-bold'>{Math.floor(props.loc.list[0].main.temp - 273.15)}&deg;C</h1>
      </section>
    </section>
  )
