...
export default function FutureWeatherReport(props = '') {
    ...
    for (let i = 0; i < Number(props.blocks); i++) {
        i == (Number(props.blocks) - 1) ? clas = `${props.paddingInBlock}` : clas = `${props.paddingInBlock} border-b-2 border-gray-600`;
        futureComponents.push(<FutureWeatherReportBlock loc={props.loc} indi={i * 7} Day={weekday[(day + i) % 7]} class={clas} key={i} />);
    }
    return (
    ...
    return (
        <section className={`${props.class} flex justify-between items-center`}>
            ...
            <section className='w-3/6 flex gap-4 justify-start items-center'>
                <img className="h-8 2xl:h-12" src={`http://openweathermap.org/img/w/${props.loc.list[props.indi].weather[0].icon}.png`} alt="" />
                <p className="w-1/6 text-white 2xl:text-2xl">{props.loc.list[props.indi].weather[0].main}</p>
            </section>
            <h1 className="2xl:text-2xl">
                {Math.floor(props.loc.list[props.indi].main.temp - 273.15)}&deg;C
            </h1>
        </section>
    )
}